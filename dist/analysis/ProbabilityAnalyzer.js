export class ProbabilityAnalyzer {
     ...
     * @private
     */
    createProbabilityResult(outcome, probability, variables = [], probabilityTree = []) {
    ...
    calculateStaticRuleProbabilities(ruleKey, visited, maxDepth, maxOutcomes, warnings) {
        ...
        for (const value of rule) {
            ...
            if (variables.length === 0) {
                ...
                const expandedOutcomes = this.expandVariablesWithProbabilities(value, variables, baseProb, visited, maxDepth, maxOutcomes, warnings);
    ...
    calculateWeightedRuleProbabilities(ruleKey, visited, maxDepth, maxOutcomes, warnings) {
        ...
        for (let i = 0; i < rule.values.length; i++) {
            ...
            if (variables.length === 0) {
                ...
                const expandedOutcomes = this.expandVariablesWithProbabilities(value, variables, probability, visited, maxDepth, maxOutcomes, warnings);
                outcomes.push(...expandedOutcomes);
            }
            if (outcomes.length >= maxOutcomes) {
                warnings.push(`Maximum outcomes (${maxOutcomes}) reached for rule '${ruleKey}'`);
    ...
    calculateTemplateRuleProbabilities(ruleKey, visited, maxDepth, maxOutcomes, warnings) {
        ...
        for (const [varName, values] of Object.entries(rule.variables)) {
            const newOutcomes = [];
            ...
            for (const currentOutcome of currentOutcomes) {
                for (const value of values) {
                    ...
                    const expandedValue = currentOutcome.value.replace(new RegExp(`%${varName}%`, 'g'), value);
                    ...
                    if (nestedVariables.length > 0) {
                        ...
                        const nestedOutcomes = this.expandVariablesWithProbabilities(expandedValue, nestedVariables, currentOutcome.probability * valueProbability, visited, maxDepth, maxOutcomes, warnings);
            ...
            if (currentOutcomes.length >= maxOutcomes)
                break;
        }
        ...
        if (remainingVariables.length > 0) {
            ...
            for (const outcome of currentOutcomes) {
                const expandedOutcomes = this.expandVariablesWithProbabilities(outcome.value, remainingVariables, outcome.probability, visited, maxDepth, maxOutcomes, warnings);
                ...
                if (finalOutcomes.length >= maxOutcomes)
                    break;
            }
            currentOutcomes = finalOutcomes;
        }
        // Convert to ProbabilityResult format
        return currentOutcomes.map(outcome => this.createProbabilityResult(outcome.value, outcome.probability));
    }
        ...
        for (const condition of rule.conditions) {
            ...
            for (const value of values) {
                ...
                if (variables.length === 0) {
                    ...
                    const expandedOutcomes = this.expandVariablesWithProbabilities(value, variables, valueProbability, visited, maxDepth, maxOutcomes, warnings);
                    outcomes.push(...expandedOutcomes);
                }
                if (outcomes.length >= maxOutcomes) {
                    warnings.push(`Maximum outcomes (${maxOutcomes}) reached for rule '${ruleKey}'`);
    ...
    expandVariablesWithProbabilities(value, variables, baseProbability, visited, maxDepth, maxOutcomes, warnings) {
        ...
        let currentOutcomes = [{ value, probability: baseProbability }];
        for (const variable of variables) {
            ...
            const variableProbabilities = this.calculateRuleProbabilities(variable, new Set(visited), maxDepth, maxOutcomes, warnings);
            // Expand each current outcome with each variable possibility
            for (const currentOutcome of currentOutcomes) {
                for (const varOutcome of variableProbabilities) {
                    const expandedValue = currentOutcome.value.replace(new RegExp(`%${variable}%`, 'g'), varOutcome.outcome);
        ...
        // Convert to ProbabilityResult format
        return currentOutcomes.map(outcome => this.createProbabilityResult(outcome.value, outcome.probability, variables, outcome.probabilityTree || []));
    }
}
