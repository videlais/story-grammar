...
describe('Parser with % symbols', () => {
    ...
    describe('Modifiers', () => {
        it('should add and apply a simple modifier', () => {
            parser.addModifier({
                name: 'uppercase',
                condition: (text) => text.includes('flower'),
                transform: (text) => text.replace(/flower/g, 'FLOWER'),
                priority: 1
            });
        ...
        it('should only apply modifiers when condition is met', () => {
            parser.addModifier({
                ...
                condition: (text) => text.includes('special'),
                transform: (text) => text + ' [MODIFIED]',
                priority: 1
            });
        ...
        it('should throw errors for invalid modifiers', () => {
            expect(() => parser.addModifier(null)).toThrow('Modifier must be an object');
            expect(() => parser.addModifier({})).toThrow('Modifier must have a name');
            expect(() => parser.addModifier({ name: 'test' })).toThrow('Modifier must have a condition function');
            expect(() => parser.addModifier({
                name: 'test',
                condition: () => true
            })).toThrow('Modifier must have a transform function');
        });
